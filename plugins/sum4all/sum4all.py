...
class sum4all(Plugin):
    def __init__(self):
        ...
        try:
            ...
            self.gemini_keys = self.keys.get("gemini_keys", [])
            ...
            # dify强制开启
            self.url_sum_enabled = True if conf().get('model') == 'dify' else self.url_sum_enabled
            ...
            # dify强制开启
            self.url_sum_enabled = True if conf().get('model') == 'dify' else self.file_sum_enabled
    ...
    def handle_url(self, content, e_context):
        ...
        if self.url_sum_service == "openai":
            ...
            api_key = random.choice(self.gemini_keys)
            model = "gemini"
            api_base = "https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateContent?key="
        else:
        ...
        if not self.url_sum_qa_enabled:
            ...
            reply.content = f"{remove_markdown(reply_content)}\n\n💬我会记忆这份链接5分钟，提问前请加【{self.url_sum_qa_prefix}】。如：{self.url_sum_qa_prefix}翻译成英文。"
        ...
        if conf().get('token_billing'):
            ...
            t = threading.Thread(target=mysql_utils.fee_deduction, args=(remark_name, 0.1,
                                                                         attr_status))
            t.start()  # 启动线程
    ...
    def handle_file(self, content, e_context):
        ...
        reply = Reply()
        reply.type = ReplyType.TEXT
        reply.content = f"{remove_markdown(reply_content)}\n\n💬我会记忆这份文档5分钟，提问前请加【{self.file_sum_qa_prefix}】。如：{self.file_sum_qa_prefix}翻译成英文。"
    ...
    def read_pdf(self, file_path):
        ...
        if content == '':
            ...
            text = pdf_extracter(file_path, force_ocr=False)
            content = str(text)
    ...
    def read_txt(self, file_path):
        ...
        try:
            ...
            logger.debug(content[:100])  # 打印文件内容的前50个字符
            return content
        except Exception as e:
    ...
    def handle_image(self, base64_image, e_context):
        ...
        if self.image_sum_service == "openai":
            ...
            api_base = "https://generativelanguage.googleapis.com/v1/models/gemini-pro-vision:generateContent"
            payload = {
                "contents": [
                    {
                        "parts": [
        ...
        reply = Reply()
        reply.type = ReplyType.TEXT
        reply.content = f"{remove_markdown(reply_content)}\n\n💬我会记忆这张图片5分钟，提问前请加【{self.image_sum_qa_prefix}】。如：{self.image_sum_qa_prefix}翻译成英文。"
